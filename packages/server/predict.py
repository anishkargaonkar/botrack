...
def main(args):
    ...
    for inst in preds.keys():
        data.utils.write_wav(os.path.join(output_folder, os.path.basename(
            args.input) + "_" + inst + ".wav"), preds[inst], args.sr)


def get_default_args(path=None):
    ...
    if path is None:
        parser.add_argument('--input', type=str, default=os.path.join("audio", "mix.mp3"),
